...
class JsonLib:
    ...
    def create_new_folder(self, foldername):
        try:
            os.makedirs(os.path.join(settings.folder_path_for_root,foldername))
            return True
    ...
    def create_json_file(self, foldername, filename):
        try:
            self.write_json_file(os.path.join(settings.folder_path_for_root, foldername, str(filename)+".json"),data=[])
            return True
        except Exception as e:
            print(e)
            return False
...
if __name__ == "__main__":
    ...
    data = script.read_json_file(
        os.path.join(settings.BASE_DIR, "output.json"))
